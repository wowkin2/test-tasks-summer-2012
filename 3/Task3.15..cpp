...
* @file         pr1.cpp
* @brief        Task3
* @task         15.	Розробити функцію, яка міняє у заданому символьному рядку 
* всі слова, що містять вказану літеру на відповідну кількість символів '*'. 
* Використовуючи розроблену функцію, "заховати" всі слова з введених речень, 
* в яких зустрічається задана літера. 
...
/**
* @brief        Sets the first num bytes pointed by a to the specified value
* @param[in]    char point to the memory to fill
* @param[in]    value to fill
* @param[in]    int count of symbol
* @return       return pointer to new string
*/
void SpaceSym(char *a, char sym, int len);
//--------------------------- main function ---------------------------//
...
int main(void)
{
	...
	sCpy(a, tmp);
	cout << "Input forbidden combination: " << endl;
	cin.getline	(tmp, 250);
	b = (char*) malloc (sizeof(char) * sLen(tmp) );
	sCpy(b, tmp);
	cout << endl;
	...
	while( len > 0 )
	{	
		if (!CheckWord(a + start, len, b))
		{
			SpaceSym(a + start, '*', len);
		}
